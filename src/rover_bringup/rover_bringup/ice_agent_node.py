#!/usr/bin/env python3
"""
ICE Agent Mode Node (NO MICROPHONE)
===================================
...
class IceAgentNode(Node):
    ...
    INITIAL_PHRASES = [
        "HALT! Show me your documentation immediately!",
        "STOP! Papers, please!",
        "You there! Don't move! Present your documents!",
        "FREEZE! Document check in progress! Show your papers!",
        "CITIZEN! Present your identification now!",
        "HEY! Where are your papers?",
    ]
    ...
    def __init__(self):
        ...
        self.cache_dir = os.path.join(tempfile.gettempdir(), 'ice_agent_cache')
        if self.cache_audio:
        ...
        self.status_pub = self.create_publisher(String, '/ice_agent/status', 10)
        self.alert_pub = self.create_publisher(Bool, '/ice_agent/alert', 10)
        self.verified_pub = self.create_publisher(Bool, '/ice_agent/document_verified', 10)
        
        # Services
        self.enable_srv = self.create_service(SetBool, '/ice_agent/enable', self.enable_callback)
        ...
        self.get_logger().info("  ICE AGENT MODE - SPEECH ONLY")
        ...
        if self.enabled:
            self.speak_threaded("ICE Agent Mode activated. Scanning for undocumented individuals.")
    ...
    def speak_elevenlabs(self, text: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                audio_path = cache_path if self.cache_audio else os.path.join(tempfile.gettempdir(), 'ice_speech.mp3')
    ...
    def enable_callback(self, request, response):
        ...
        if self.enabled:
            self.speak_threaded("ICE Agent Mode reactivated. Resuming patrol.")
        else:
            self.speak_threaded("Standing down. Patrol suspended.")
            self.cmd_vel_pub.publish(Twist())
            self.person_detected = False
        response.success = True
        response.message = f"ICE Agent {'enabled' if self.enabled else 'disabled'}"
        return response
